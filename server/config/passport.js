    ...
    User.findOne({ email: email.toLowerCase() }, (err, existingUser) => {
        ...
        if(!existingUser.password) {
            return done(null, false, 
                { msg: '소셜 로그인은 비밀번호 설정을 제공하지 않습니다.'});
        }
        /**
         * 유저가 있다면, 비밀번호 비교
...
const naverStrategyConfig = new NaverStrategy({
    ...
    console.log('액세스 토큰', accessToken);//이걸 받아야 profile정보가 나오고
    console.log('리프레시 토큰', refreshToken);//이걸 받아야 params정보가 나온다.
    console.log('파라미터', params);//params 안에 accessToken있음
    ...
    User.findOne({ naverId: profile.id }, (err, existingUser) => {
        ...
        User.findOne({ email: profile.emails[0].value }, (err, existingUser) => {
            ...
            if(existingUser) {
                ...
                newUser.name = profile.displayName;
                newUser.image = profile._json.profie_image;
